...
const E={};
  ...
  E.phaseDecision=function(concern,levels,{sensitive=false}={}){
    ...
    if((concern==='acne'&&pigmentActive)||(concern==='pigment'&&acneActive)){
      return {kind:sensitive?'acne_pigment_shared_sensitive':'acne_pigment_shared_robust',title:sensitive?'Lower-irritation shared phase for blemishes + pigmentation':'Shared phase for blemishes + pigmentation',covers:['acne','pigment']};
    }
  ...
  E.basePolicy=function({baumann,moisture,levels}){
    ...
    const needsLeaveOnTreatment=['acne','pigment','congestion','ageing'].some(k=>(levels?.[k]||1)>=3);
    const cleanserRole=(D||S||levels.barrier>=3)?'cleanser_sensitive':(needsLeaveOnTreatment?'cleanser_oily_treatment_safe':'cleanser_oily');
